...
use {String as S, Vec as V, Box as B,
     std::{option::Option as O, collections::HashMap as M, error::Error,
...
macro_rules! er {($s:expr)=>{Err(Er::n($s))}}
...
impl Debug for A { fn fmt(&self, f: &mut FF<'_>) -> FR { use crate::A::*; match &self {
  A(_)|B(_)|N(_)|S(_)|L(_) => {f.write_str(&format!("{:?}",self))},F(_) => {f.write_str("#'fn")}}}}
impl Display for A { fn fmt(&self, f: &mut FF<'_>) -> FR {
  let r = match self {
    A::A(x) => x.clone(),A::B(x) => x.to_string(),A::N(x) => x.to_string(),
    A::S(x) => format!("\"{}\"",x),A::F(_) => "#'fn".to_string(),
...
struct E {env:M<S,A>,pub out: O<B<E>>} // TODO out
impl E {
...
fn parses(s:&A) -> R<S> {match s {A::S(s) => Ok(s.clone()), _ => er!("expected string")}}
fn parse<'a>(i:&'a[S]) -> R<(A,&'a[S])> {
  let (car, cdr) = i.split_first().ok_or(Er::n("could not get token"))?;
  match &car[..] {"(" => seq(cdr),")" => er!("unexpected `)`"),_ => Ok((atom(car), cdr))}}
		 ...
		 None => {let fx = eval(car,e)?;
			  match fx {
			    A::F(f) => {
			      ...
			      f(&rx?)},
			    _ => er!("first form must be a function")}}}},
  _ => er!("unexpected form")}}
fn evala(car:&A,cdr:&[A],e:&mut E) -> O<R<A>> {match car {
  A::A(x) => match x.as_ref() {
    "?" => Some(eif(cdr,e)),":" => Some(ede(cdr,e)),
    _ => None}, _ => None}}
fn eif(i:&[A],e:&mut E) -> R<A> {
  ...
  match tx {
    A::B(x) => {
      ...
      eval(r,e)},_ => er!(&format!("unexted cond form: {}", t.to_string()))}}
fn ede(i:&[A],e:&mut E) -> R<A> {
  let d0 = i.first().ok_or(Er::n("expected first form"))?;
  let ds = match d0 {A::A(x) => Ok(x.clone()), _ => er!("expected first form to be symbol")}?;
  let d1 = i.get(1).ok_or(Er::n("expected second form"))?;
  if i.len()>2 {return er!("def can only have two forms")}
  let dx = eval(d1,e)?;e.set(&ds,dx);Ok(d0.clone())}
fn rl()->S {let mut s = S::new(); stdin().read_line(&mut s).expect("failed to read line"); s}
fn repl(e:&mut E) -> R<()> { loop {print!("|| ");stdout().flush().or(er!("failed to display prompt"))?;
