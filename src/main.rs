...
macro_rules! ab {($f:expr) => {{|i: &[A]| -> R<A> { //comp
  ...
  Ok(A::B(f(car,cdr)))}}}}
#[derive(Default, Clone,PartialEq,PartialOrd)] struct G{args:Rc<A>,body:Rc<A>} //λ
...
fn ld<P:AsRef<Path>>(p:P,e:&mut E,i:bool)-> R<()> { //load file
  println!("{:?}",p.as_ref());
  let f = BufReader::new(F::open(p.as_ref()).or(err!("failed to open file"))?).lines();
...
fn repl(e:&mut E) -> R<()> {
  ...
  loop { print!("|| ");stdout().flush().or(err!("failed to display prompt"))?;
	 let i = rl()?; if let Ok((r,_)) = parse(&tok(i)) {
	   match eval(&r, e) {Ok(r) => println!("  {}", r),Err(e) => eprintln!("{}", e)}} else {continue}}}

fn main() -> R<()> {
  let a:V<S>=args().skip(1).collect();let mut cc=false;let mut e=E::n(None);let e = e.init(); for i in &a {
